...
library(tidyverse)
library(dplyr)
gather(df, age, val, age_15_17:age_23up) %>% 
       filter(val!=0) %>%
       select(-val)
```
...
```{r}
library(BMA)
#glm_0<- bic.glm(df[,-c(1,2)], df[,1], glm.family="binomial", factor.type=FALSE)

summary(glm.out.FT)
imageplot.bma(glm.out.FT)

library(arm)
fit <- bayesglm(df[,1] ~ df[,-c(1,2)], data=df, family="binomial")

library(glmnet)
fit2=glmnet(df[,-c(1,2)],df[,1],family="binomial")

df_binary<- df[,-2]
glm_model <- glm(df$drinks_binary~ .,data = df_binary, family = binomial)
summary(glm_model)
