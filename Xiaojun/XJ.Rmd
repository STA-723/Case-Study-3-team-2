...
df_simple$marital<- factor(as.matrix(df[,c(27:30)])%*% 1:4, labels = colnames(df[,c(27:30)]))
df_simple$race<- factor(as.matrix(df[,c(31:36)])%*% 1:6, labels = colnames(df[,c(31:36)]))
df_simple$religion<- factor(as.matrix(df[,c(37:42)])%*% 1:6, labels = colnames(df[,c(37:42)]))
df_simple<- cbind(df_simple, df[,43:47])
#df_simple$max_par_edu<- factor(as.matrix(df[,c(48:50)])%*% 1:3, labels = colnames(df[,c(48:50)]))
```
...
fit <- bayesglm(df_simple_bayes[,1] ~ ., data=df_simple_bayes, family="binomial")
summary(fit)

library(glmnet)
x <- model.matrix(df_simple[,1]~., df_simple)[,-1]
...
```{r}
df_nb_true0<- rbind(df_simple[(df_simple$num_of_drinks ==0 & df_simple$df.drinks_binary ==0), ], df_simple[(df_simple$num_of_drinks !=0 & df_simple$df.drinks_binary !=0), ])
df_nb_true0$num_drink_true0<- ceiling(df_nb_true0$num_drink_true0)

vglm(formula = df_simple$num_drink_true0 ~ df_simple[,3:], family = posnegbinomial(), data = df_nb_true0)
